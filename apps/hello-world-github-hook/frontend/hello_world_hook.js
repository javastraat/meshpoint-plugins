 ...
 * (apps/hello-world-github/). See frontend/sidebar/page_hook_registry.js
 * for the seam itself and docs/PLUGINS.md for the full writeup.
 *
 * The target host is plugin.toml's [hook].host ("hello-world-github") --
 * must match the host plugin's own [sidebar].route exactly, and the host's
 * own frontend script must call window.mountPageHooks() from its mount()
 * for this to ever actually render (see hello_world.js).
 */
