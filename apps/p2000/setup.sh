#!/usr/bin/env bash
#
# P2000 plugin dependency: multimon-ng, built from source.
#
#   * EliasOenal/multimon-ng -- decodes FLEX (P2000's protocol) out of
...
#
# NOTE: Pagers and POCSAG (still built into core, not yet plugins) use
# this exact same multimon-ng binary -- scripts/install.sh already
# builds it unconditionally as part of RTL-SDR setup, so on a normal Pi
# install this script is a no-op. It stays self-contained anyway so the
# P2000 plugin doesn't silently depend on Pagers/POCSAG still being core.
#
...
#     sudo bash plugins/apps/p2000/setup.sh
set -euo pipefail
